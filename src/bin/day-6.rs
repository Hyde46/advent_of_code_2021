...
fn simulate_cached_fish(input: &str, days: usize) -> usize {
    ...
    fish.iter().for_each(|f| fish_cache[*f as usize] = fish_cache[*f as usize] + 1);
    for _i in 1..days + 1 {
        fish_cache.iter().enumerate().for_each(|(i, f)| {
            let target_swap_index = if i == 0 {6} else {i - 1};
            fish_swap[target_swap_index] = if fish_swap[target_swap_index] == 0 {*f} else {*f + fish_swap[target_swap_index]};
            if i == 0 {
                fish_swap[8] = *f;
            }
        });
...
fn main() {
    println!("Cached fish simulation {}", simulate_cached_fish(INPUT, 256));
}
